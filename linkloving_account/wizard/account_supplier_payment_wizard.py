...
class AccountEmployeeRegisterPaymentWizard(models.TransientModel):
    _name = "account.supplier.payment.wizard"
    _description = "Account Supplier Payment Wizard"

    @api.model
    def _default_payment_id(self):
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        payment = self.env['account.payment.register'].browse(active_ids)
        return payment.id

    register_id = fields.Many2one('account.payment.register', required=1, default=_default_payment_id)
