    ...
    if ($("#" + tree_id).length > 0) {
        ...
        for (var i = 0, l = newNodes.length; i < l; i++) {
            ...
            if (node.modify_type && node.modify_type == 'delete') {
                ...
                body_html += "<tr data-product-id='" + node.productid + "'data-to-add='" + node.to_add + "'data-product-spec='" + node.product_spec + "'data-qty='" + node.qty + "' class='" + new_bg + "' data-tt-id='" + node.id + "' " + (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") + "><td class = 'td_left'><span data-pt-id='" + node.ptid + "' class='" + (node.isParent ? "parent" : "children") + " product_name'>" + node.name + "</span></td>";
