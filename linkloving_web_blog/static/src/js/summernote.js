    ...
    var Editor = function (context) {
        ...
        this.createLink = this.wrapCommand(function (linkInfo) {
            ...
            if (options.onCreateLink) {
                ...
                // var sear = new RegExp('/web/image/');
                if (!$('.note-link-attachment').val()) {
                    linkUrl = /^[A-Za-z][A-Za-z0-9+-.]*\:[\/\/]?/.test(linkUrl) ?
                        linkUrl : 'http://' + linkUrl;
                } else $('.note-link-attachment').val('')
            }
                ...
                ui.onDialogShown(self.$dialog, function () {
                    ...
                    $linkAttachment.on('change', function () {
                        ...
                        reader.onload = function (e) {
                            ...
                            UploadFiles($linkAttachment.val(), e.target.result, function (url) {
                                var name_list = $linkAttachment.val().split("\\");
                                $linkText.val(name_list[name_list.length - 1]);
                                $linkUrl.val(url);//将上传后的URL赋值到linkUrl的输入框中
                                $linkBtn.toggleClass('disabled', false);
...
$(function () {
    ...
    $("button.dropdown-toggle").on("click", "button", function (e) {
