...
class OpenAccountChart(models.TransientModel):
    ...
    def account_chart_open_window(self):
        ...
        if self.parent_needed:
            result = self.env.ref('linkloving_account_parent.open_view_account_tree').read([])[0]
        else:
            result = self.env.ref('linkloving_account_parent.open_view_account_noparent_tree').read([])[0]
