...
class LinklovingOAApi(http.Controller):
    ...
    def get_products(self, *kw):
        ...
        if request.jsonrequest.get("name"):
            ...
            products = request.env['product.product'].sudo().search(
                ['|', ('name', 'ilike', name), ('default_code','ilike',name), ('sale_ok','=',True)], limit=10, offset=0, order='id asc')
        else:
    ...
    def get_customer_details(self, obj):
        data = {
            ...
            'supplier': request.env['account.invoice'].sudo().search_count([('partner_id', '=', request.jsonrequest.get(
                "id"))]),
            ...
            'return_count': request.env['stock.picking'].sudo().search_count(
                [('partner_id', '=', request.jsonrequest.get("id")), ('state', '=', 'waiting_in')]),  #退货入库数量
    ...
    def get_waiting_approval_detail_lists(self, objs):
        ...
        for obj in objs:
            data.append({
                ...
                # 'tax_ids':obj.tax_ids,
                'name': obj.name or '',
                'description': obj.compyter_body or '',
            })
        return data
