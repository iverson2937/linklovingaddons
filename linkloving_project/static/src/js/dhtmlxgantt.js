...
gantt._init_grid = function () {
    ...
    this._click.gantt_add = this.bind(function (e, id, trg) {
        ...
        this.callEvent("onTaskAdd", [ id ? id : this.config.root_id]);
...
gantt.updateTask = function (id, item) {
    ...
    if (this.callEvent("onBeforeTaskUpdate", [item, item]) === false)
        return false;
...
gantt.addLink = function (link) {
    ...
    this.callEvent("onBeforeLinkAdd", [link, link])
    // this._lpull[link.id] = link;
    // this._sync_links();
    // this._render_link(link.id);
    // this.callEvent("onAfterLinkAdd", [link, link]);
    // return link.id;
};
...
gantt._deleteLink = function (id, silent) {
    var link = this.getLink(id);
    if (!silent && this.callEvent("onBeforeLinkDelete", [link, link]) === false) return false;
...
gantt._init_html_area = function (node) {
    if (typeof node == "string")
        this._obj = document.getElementsByClassName(node)[0];
    else
        this._obj = node;
    console.log($(".o_view_manager_content"));
    console.log($(".gantt_container"));
    // $(".o_view_manager_content").html("222");
